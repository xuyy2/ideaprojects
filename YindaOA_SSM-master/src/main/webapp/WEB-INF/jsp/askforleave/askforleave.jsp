   ...
   <div class="head-cont">
        ...
        <div class="head-nav fl" id="h-nav">
            <ul>
                ...
                <li><a class="active"href="<%=path%>/toAskForLeave.do">请假明细</a>
...
<form id="form"> 
  <div class="caidan-tiku" style="margin-bottom:3%">              
              <div class="caidan-tiku-s" style="margin-right:5%"> <span>姓名：</span>
                    <input id="search_name" type="text" name="yoAskStaffName" class="shuruk-a2" placeholder="" value="${askForLeave2.yoAskStaffName}">
                </div>
                <div class="caidan-tiku-s" style="margin-right:5%"> <span>工号：</span>
                    <input id="search_workid" type="text" name="yoAskStaffId" class="shuruk-a2" placeholder="" value="${askForLeave2.yoAskStaffId }">
                </div>
                <div class="caidan-tiku-s" style="margin-right:5%"> <span>部门：</span>
                    <select id="yoAskStaffDepart" name="yoAskStaffDepart" class="shuruk-a2" placeholder="" >
                    	<option value=""></option>
                ...
                <div class="caidan-tiku-s" style="margin-right:5%"> <span>审批结果：</span>
                    <select id="yoApproveResult" name="yoApproveResult" class="shuruk-a2"  >
                    	<option value="同意" <c:if test="${askForLeave2.yoApproveResult eq '同意'}">selected="selected"</c:if>>同意</option>
                    	<option value=""  <c:if test="${askForLeave2.yoApproveResult eq ''}" >selected="selected"</c:if>>全部</option>
                    </select>
                </div>
                <div class="caidan-tiku-s" style="margin-right:5%"> <span>时间：</span>
                    <input id="start" type="text" name="yoAskBeginDate" class="laydate-icon shuruk-a2 form_date" placeholder="" value="${askForLeave.yoAskBeginDate }">
                    <input id="search_name2" type="text" name="yoAskEndDate" class="shuruk-a2 form_date" placeholder=""  value="${askForLeave.yoAskEndDate }">
                </div>
    ...
    <div style="width:100%; height:700px;padding-top: 5px;overflow:auto;border:0 solid #000000;">
            <table  width="100%" border="1" cellspacing="0" cellpadding="0" class="table-1">
                ...
                <td class="text_center" width="6%">请假类型</td>
                <td class="text_center" width="6%">开始时间</td>
                <td class="text_center" width="6%">结束时间</td>
                <td class="text_center" width="6%">请假天数</td>
                <td class="text_center" width="6%">请假类型</td>
                <td class="text_center" width="6%">请假事由</td>
                <td class="text_center" width="6%">审批人</td>
                <td class="text_center" width="6%">审批状态</td>
                </thead>
                <tbody >
               <c:forEach items="${askForLeaveList}" var="askForLeave">
                <tr >
                 <td class="text_center" width="6%">${askForLeave.yoAskStaffName }</td>
                <td class="text_center" width="6%">${askForLeave.yoAskStaffDepart }</td>
                <td class="text_center" width="9%">${askForLeave.yoAskStaffId }</td>
                <td class="text_center" width="6%">${askForLeave.yoType}</td>
                <td class="text_center" width="6%">${askForLeave.yoAskBeginDate }</td>
                <td class="text_center" width="6%">${askForLeave.yoAskEndDate}</td>
                <td class="text_center" width="6%">${askForLeave.yoAskSustain }</td>
                <td class="text_center" width="6%">${askForLeave.yoType}</td>
                <td class="text_center" width="6%">${askForLeave.yoAskReason}</td>
                <td class="text_center" width="6%">${askForLeave.yoHistoryApproveName }</td>
                <td class="text_center" width="6%">${askForLeave.yoApproveResult }</td>
                </tr>
               </c:forEach>
                </tbody>
            </table>
...
$('.form_date').datetimepicker({
	...
	function clearPropertities(){
		$("input[name='yoAskStaffName']").val("");
		$("select[name='yoAskStaffDepart']").val("");
		$("input[name='yoAskStaffId']").val("");
		$("input[name='yoAskBeginDate']").val("");
		$("input[name='yoAskEndDate']").val("");
	}
	function downLoad(){
		window.location.href="askForLeaveDownload.do";
	}
</script>
</body>
</html>
