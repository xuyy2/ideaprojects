                ...
                <div class="weui_cell weui_cell_select">
                    <div class="weui_cell_bd weui_cell_primary">
                        <select class="weui_select" name="orderName" id="orderName">
                            <option value="${itemChange.orderName}">${itemChange.orderName}</option>
                        </select>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function pass_approve(){
        ...
        $.post("<%=path%>/orderChange/pass_approve.do",{"id":id,"identify":yindaIdentify,"orderRemark":orderRemark,
        	"businessProp":businessProperty,"outdoorJob3":outdoorJob,"lte3":lte,"principal3":principal},function(data){
            if("success"==data){
                $.alert("操作成功！");
                window.history.go(-1);
            }else{
